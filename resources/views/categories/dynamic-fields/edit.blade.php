...
@section('title', 'Edit Field — Servixa Admin')
...
@section('breadcrumb')
    ...
    <svg class="w-4 h-4 text-[#6B7280] rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-[#1F2937] font-medium">Edit Field</span>
@endsection
...
<div class="max-w-2xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#1F2937]">Edit Dynamic Field</h1>
        <p class="text-sm text-[#6B7280] mt-1">Update field configuration</p>
    </div>

    <x-card>
        <form method="POST" action="{{ route('admin.categories.fields.update', [$category, $field]) }}" class="space-y-6"
              x-data="{ fieldType: '{{ old('field_type', $field->field_type) }}' }">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-input
                        name="label_en"
                        label="English Label"
                        :value="old('label_en', $field->getTranslation('label', 'en'))"
                    />
                    <div>
                        ...
                        <input
                            type="text"
                            name="label_ar"
                            value="{{ old('label_ar', $field->getTranslation('label', 'ar')) }}"
                            dir="rtl"
            ...
            <div>
                ...
                <select
                    name="field_type"
                    x-model="fieldType"
                    class="w-full px-3 py-2.5 text-sm border border-[#DDD6FE] rounded-xl bg-white text-[#1F2937] focus:outline-none focus:ring-2 focus:ring-[#6B21A8]/30 focus:border-[#6B21A8] transition"
                >
                    ...
                    <option value="select" {{ old('field_type', $field->field_type) === 'select' ? 'selected' : '' }}>Select (Dropdown)</option>
                    <option value="textarea" {{ old('field_type', $field->field_type) === 'textarea' ? 'selected' : '' }}>Textarea</option>
                    <option value="boolean" {{ old('field_type', $field->field_type) === 'boolean' ? 'selected' : '' }}>Boolean (Yes/No)</option>
                </select>
            </div>
            ...
            <div x-show="fieldType === 'select'" x-cloak>
                <label class="block text-sm font-medium text-[#1F2937] mb-1.5">
                    Options <span class="text-[#6B7280] font-normal">(one per line)</span>
                </label>
                <textarea
                    name="options_raw"
                    rows="5"
                ...
                >{{ old('options_raw', !empty($field->options) ? implode("\n", $field->options) : '') }}</textarea>
                @error('options_raw')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
