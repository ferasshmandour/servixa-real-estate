...
@section('title', __('admin.city_create_title') . ' — Servixa Admin')
...
@section('breadcrumb')
    ...
    <span class="text-[#1F2937] font-medium">{{ __('admin.city_create_title') }}</span>
@endsection
...
<div class="max-w-lg">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#1F2937]">{{ __('admin.city_create_title') }}</h1>
        <p class="text-sm text-[#6B7280] mt-1">{{ __('admin.city_create_subtitle') }}</p>
    </div>

    <x-card>
        <form method="POST" action="{{ route('admin.cities.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-[#F5F3FF]">
                ...
                <x-button variant="primary" type="submit">{{ __('admin.city_create_btn') }}</x-button>
            </div>
        </form>
    </x-card>
</div>
