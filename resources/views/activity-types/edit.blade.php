...
@section('title', __('admin.at_edit_title') . ' — Servixa Admin')
...
@section('breadcrumb')
    ...
    <span class="text-[#1F2937] font-medium">{{ __('admin.at_edit_title') }}</span>
@endsection
...
<div class="max-w-lg">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#1F2937]">{{ __('admin.at_edit_title') }}</h1>
        <p class="text-sm text-[#6B7280] mt-1">{{ __('admin.at_edit_subtitle') }}</p>
    </div>

    <x-card>
        <form method="POST" action="{{ route('admin.activity-types.update', $activityType) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <x-input
                ...
                :value="old('name_en', $activityType->getTranslation('name', 'en'))"
            />
            ...
            <div>
                ...
                <input
                    type="text"
                    name="name_ar"
                    value="{{ old('name_ar', $activityType->getTranslation('name', 'ar')) }}"
                    dir="rtl"
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-[#F5F3FF]">
                ...
                <x-button variant="primary" type="submit">{{ __('admin.at_save_btn') }}</x-button>
            </div>
        </form>
    </x-card>
</div>
